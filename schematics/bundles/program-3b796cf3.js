...
function compileDeclareClassMetadata(metadata) {
    ...
    definitionMap.set('version', checker.literal('19.1.0-next.0+sha-899f711-with-local-changes'));
...
class ImportedSymbolsTracker {
     ...
     */
    findClassImports(sourceFile, classDeclaration) {
     ...
     */
    isClassImported(sourceFile, classDeclaration) {
...
function extractDirectiveMetadata(clazz, decorator, reflector, importTracker, evaluator, refEmitter, referencesRegistry, isCore, annotateForClosureCompiler, compilationMode, defaultSelector, strictStandalone) {
    ...
    if (decorator.args === null || decorator.args.length === 0) {
        ...
        meta = checker.unwrapExpression(decorator.args[0]);
        if (!ts__default["default"].isObjectLiteralExpression(meta)) {
    ...
    if (!directive.has('selector') && decorator.import?.name && meta && selector === 'ng-component') {
        ...
        if (decoratorName === 'Component') {
            ...
            if (ts__default["default"].isObjectLiteralExpression(meta)) {
                ...
                ts__default["default"].factory.updateObjectLiteralExpression(meta, [...meta.properties, selectorProp]);
...
class ComponentDecoratorHandler {
    constructor(reflector, evaluator, localMetaRegistry, metaRegistry, metaReader, scopeReader, compilerHost, scopeRegistry, typeCheckScopeRegistry, resourceRegistry, isCore, strictCtorDeps, resourceLoader, rootDirs, defaultPreserveWhitespaces, i18nUseExternalIds, enableI18nLegacyMessageIdFormat, usePoisonedData, i18nNormalizeLineEndingsInICUs, moduleResolver, cycleAnalyzer, cycleHandlingStrategy, refEmitter, referencesRegistry, depTracker, injectableRegistry, semanticDepGraphUpdater, annotateForClosureCompiler, perf, hostDirectivesResolver, importTracker, includeClassMetadata, compilationMode, deferredSymbolTracker, forbidOrphanRendering, enableBlockSyntax, enableLetSyntax, externalRuntimeStyles, localCompilationExtraImportsTracker, jitDeclarationRegistry, i18nPreserveSignificantWhitespace, strictStandalone, enableHmr) {
        this.reflector = reflector;
        this.evaluator = evaluator;
        this.localMetaRegistry = localMetaRegistry;
        this.metaRegistry = metaRegistry;
        this.metaReader = metaReader;
        this.scopeReader = scopeReader;
    ...
    resolve(node, analysis, symbol) {
        ...
        if (scope !== null) {
            ...
            if (metadata.isStandalone) {
                for (const directivesClass of this.localMetaRegistry.getKnown(checker.MetaKind.Directive)) {
                    if (this.importTracker.findClassImports(node.getSourceFile(), directivesClass).found) {
                        const res = this.metaReader.getDirectiveMetadata(new checker.Reference(directivesClass));
                        if (res) {
                            dependencies.push(res);
                        }
                    }
                }
                for (const directivesClass of this.localMetaRegistry.getKnown(checker.MetaKind.NgModule)) {
                    if (this.importTracker.findClassImports(node.getSourceFile(), directivesClass).found) {
                        const res = this.metaReader.getNgModuleMetadata(new checker.Reference(directivesClass));
                        if (res) {
                            dependencies.push(res);
                        }
                    }
                }
                for (const directivesClass of this.localMetaRegistry.getKnown(checker.MetaKind.Pipe)) {
                    if (this.importTracker.findClassImports(node.getSourceFile(), directivesClass).found) {
                        const res = this.metaReader.getPipeMetadata(new checker.Reference(directivesClass));
                        if (res) {
                            dependencies.push(res);
                        }
                    }
...
class PipeDecoratorHandler {
    ...
    analyze(clazz, decorator) {
        ...
        if (!pipe.has('name')) {
            ...
            pipe.set('name', ts__default["default"].factory.createStringLiteral(classNameWithoutSuffix.charAt(0).toLowerCase() + classNameWithoutSuffix.slice(1), true));
        }
...
function coreVersionSupportsFeature(coreVersion, minVersion) {
    // A version of `19.1.0-next.0+sha-899f711-with-local-changes` usually means that core is at head so it supports
